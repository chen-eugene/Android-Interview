...
#### [14、FragmentPager、FragmentStatePagerAdapter区别。](http://www.apkbus.com/android-90417-1-1.html)
  ...
  FragmentPagerAdapter适用于Fragment比较少的情况，因为会把每一个Fragment保存在内存中，不用每次切换的时候，去保存现场，切换回来在重新创建，所以用户体验比较好。FragmentStatePagerAdapter适用于Fragment比较多的情况，切换的时候销毁以前的Fragment以释放内存。
