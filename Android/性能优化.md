...
#### 13、怎么发现和分析内存泄漏。
工具：
- [Android Memory Monitor](https://blog.csdn.net/self_study/article/details/66969064)：AS自带的内存监测工具。
- MAT：它是一个快速、功能丰富的 JAVA heap 分析工具，它可以帮助我们查找内存泄漏和减少内存消耗。
- adb shell && Memory Usage 命令：可以通过命令 adb shell dumpsys meminfo [package name] 来将指定 package name 的内存信息打印出来，这种模式可以非常直观地看到 Activity 未释放导致的内存泄漏。
- [Allocation Tracker](https://blog.csdn.net/itfootball/article/details/48750849)：Android studio 还自带一个 Allocation Tracker 工具。
- LeakCanary
